...
class FlatCacher:
    ...
    def __init__(self, logger, flatDir):
        ...
        if not os.path.exists(flatDir):
            try:
                ...
                msg = 'flat output dir {} does not exist and cannot be created'.format(
                    flatDir)
    ...
    def getFlat(self, fns):
        ...
        # If reduced flat exists on disk, then restore it, cache it and return
        # it
...
#             ', '.join(str(x) for x in ([fn[fn.find("NS"):fn.rfind(".")] for fn in fns])))
    ...
    def constructFlatName(self, baseFns):
        ...
        """
        for n in range(8):
            fn = sorted(baseFns)[0][:sorted(baseFns)[0].find(
                '.fits')] + '_flat_' + str(n) + '.fits'
    ...
    def combineFlats(self, fns):
        ...
        if len(fns) < 3:
            if config.params['no_cosmic']:
                self.logger.info(
                    'flat cosmic ray cleaning inhibited by command line flag')
            else:
                self.logger.info(
                    'cosmic cleaning flat because < 3 flats were median combined')
