...
def line_id(order, oh_wavelengths, oh_intensities):
    ...
    logger.info('wavelength scale shift = ' +
                str(round(order.waveShift, 3)) + ' pixels')
    ...
    if id_tuple is not None:
        ...
        matchesdx, matchesohx, matchesidx = np.array(
            []), np.array(
            []), np.array(
            [])
...
#             matchesidx = np.concatenate((matchesidx, id_tuple[2]))
    ...
    # polyfit() returns highest power polynomial coefficient first, so p[0] is
    # slope.
    disp = p[0]

    if DISP_UPPER_LIMIT > abs(disp) > DISP_LOWER_LIMIT:
        logger.info(
            'slope of accepted vs measured wavelengths = ' + str(round(disp, 3)))
        ...
        logger.warning(
            'per-order wavelength fit slope out of limits, not using sky lines from this order')
        return None
...
def get_oh_lines():
    ...
    try:
        ...
        try:
            ...
            logger.error(
                'failed to open OH emission line file: ' +
                oh_filename)
            raise
...
def gen_synthesized_sky(oh_wavelengths, oh_intensities, wavelength_scale_calc):
    ...
    for i in np.arange(0, x.size):
        if y[i] > 0.01:
            g = y[i] * np.exp(-(wavelength_scale_calc - x[i])
                              ** 2.0 / (2.0 * GAUSSIAN_VARIANCE ** 2.0))
            all_g = all_g + g
...
def find_wavelength_shift(sky, gauss_sky, grating_eq_wave_scale):
    ...
    # ohg is a synthetic spectrum of gaussians
    ohg = np.array([grating_eq_wave_scale, gauss_sky])
...
#     pl.show()
    ...
    # only look at the part of sky line list that is around the theory
    # locations
    ...
    if bigohy.any():
        ...
        # there were no sky lines in the table that match theoretical
        # wavelength range
        logger.info(
            'could not find known sky lines in expected wavelength range')
        return []
...
#         return []
    ...
    if bigohx.any() and bigdx.any():
        ...
        bigohy2 = bigohy
        bigidx2 = bigidx
        # happened is a counter of doublets matched, removed from bigdx, bigohx
        # and added to match list
        happened = 0
            ...
            if bigdx[i + 1] - bigdx[i] < 2:
                ...
                if len(
                    ...
                    if len(locx) > 2:
                        ...
                        yslice = np.array(
                            bigohy2[locx[0] - 2 * happened:locx[-1] - 2 * happened + 1])
                        ...
                        # remove the max from yslice
                        yslice = np.delete(yslice, locxfix[0])
                    ...
                    if len(ohslice) > 1:
                        for j in range(0, 1):
                            ...
                            if ((ohslice[j + 1] - ohslice[j]) < 2 and abs(ohslice[j] - bigdx2[i - 2 * happened]) < 6
                                ...
                                matchesohy.append(
                                    bigohy2[locx[0] - 2 * happened + j])
                                matchesohy.append(
                                    bigohy2[locx[0] - 2 * happened + j + 1])
                                ...
                                bigdx2 = np.delete(
                                    bigdx2, i - 2 * happened)  # this removes the "i+1"
                                bigohx2 = np.delete(
                                    bigohx2, locx[0] - 2 * happened + j)
                                bigohx2 = np.delete(
                                    bigohx2, locx[0] - 2 * happened + j)  # this removes the "j+1"
                                bigohy2 = np.delete(
                                    bigohy2, locx[0] - 2 * happened + j)
                                bigohy2 = np.delete(
                                    bigohy2, locx[0] - 2 * happened + j)  # this removes the "j+1"
...
# return [matchesdx, matchesohx, matchesohy, bigohx, bigohy, 1,
# matchesidx]
...
#     raw_input('waiting')
    ...
    return np.ravel(a0 + a1 * x + a2 * x ** 2 + a3 * y +
                    a4 * x * y + a5 * (x ** 2) * y - f)
...
#     newoh = 9999
    ...
    outliers = ((x_res[i], residual[i])
                for i, t in enumerate(test.iloc[:, 2]) if t < 0.9)
...
#                          label=str(k) + ' fit')
        ...
        if sigma > SIGMA_MAX and len(dataZ_new) > LOWER_LEN_POINTS:
            ...
            if testing:
                dataZ_new_forplot[residual.argmax() -
                                  happened] = dataZ_new_forplot[residual.argmin()]
                dataX_new_forplot[residual.argmax() -
                                  happened] = dataX_new_forplot[residual.argmin()]
                dataY_new_forplot[residual.argmax() -
                                  happened] = dataY_new_forplot[residual.argmin()]
...
#                 exit()
    ...
    logger.info('wavelength calibration rms fit residual = ' +
                str(round(sigma, 3)))

    logger.info('wavelength eq coefficients: ' +
                ', '.join('{:.2E}'.format(k) for k in p1))
