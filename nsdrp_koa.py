...
def process_dir(in_dir, base_out_dir):
    ...
    # use main logger for outer logging, reduce_frame makes the per frame
    # logger
    ...
    # instantiate a flat cacher so each flat or set of flats is only reduced
    # once
        ...
        if nirspecConfig is None:
            ...
            if nirspecConfig.isTheSame(rawDataSet.objHeader) is not None:
                ...
                if len(reducedDataSets) > 0:
                    ...
                    gen_data_products(
                        reducedDataSets, nirspecConfig, base_out_dir, ssFptr)
...
#     logger.info('running mcal')
    ...
    if allCalibrated is True:
        logger.info(
            'all frames have been calibrated, no adjacent frame calibration required')
        return
...
def start_summary_ss(in_dir, out_dir):
    ...
    if config.params['ut'] is not None:
        ...
        for fn in fns:
            if fn.startswith('NS.'):
                ss_fn = out_dir + '/' + \
                    fn[:fn.find('.', fn.find('.') + 1)] + '_summary.csv'
                break
        if ss_fn is None:
