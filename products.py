...
def constructFileName(outpath, base_name, order, fn_suffix):
    ...
    if order is None:
        ...
        return fn[:fn.rfind(base_name) + len(base_name) + 1] + \
            str(order) + fn[fn.rfind(fn_suffix) - 1:]
...
def gen(reduced, out_dir):
    ...
    """
    obj_logger.info(
        'generating data products for {}...'.format(
            reduced.getBaseName()))
    ...
    for i in range(len(reduced.flatKOAIds)):
        header['FLAT' + str(i)] = (reduced.flatKOAIds[i],
                                   'KOAID of flat {}'.format(i))
    ...
    for order in reduced.orders:
        ...
        header['OBJEXTRW'] = (obj_window_w,
                              'width of object extraction window in pixels')
        ...
        if order.isPair:
            ...
            try:
                if len(order.topSkyWindow) > 0:
                    header['SKYDIST'] = (
                        order.topSkyWindow['A'][0] - order.objWindow['A'][-1], SKYDIST_INFO)
                else:
                    header['SKYDIST'] = (
                        order.objWindow['A'][0] - order.botSkyWindow['A'][-1], SKYDIST_INFO)
        ...
        header['PROFPEAK'] = (
            round(
                centroid,
                3),
            'fractional row number of profile peak')
        ...
        for frame in reduced.frames:
            ...
            spectrumPlot(out_dir, reduced.baseNames[frame], 'snr', order.flatOrder.orderNum,
                         '', np.absolute(
                order.objSpec[frame] / order.noiseSpec[frame]), order.waveScale,
                order.calMethod)
...
def profilePlot(outpath, base_name, order_num, profile, peak, centroid,
    ...
    if peak > (len(profile) / 2):
        pl.annotate(aStr, (peak - (len(ext_range) / 2) -
                           20, ((ymax - ymin) * 3 / 5) + ymin))
    else:
        pl.annotate(aStr, (peak + (len(ext_range) / 2) +
                           5, ((ymax - ymin) * 3 / 5) + ymin))
...
def fitsSpectrum(outpath, base_name, title, order_num,
                 y_units, cont, wave, header):
...
def multiSpectrumPlot(outpath, base_name, order,
                      y_units, cont, sky, noise, wave):
    ...
    pl.plot(wave[:1004], cont[:1004], "k-", mfc="none",
            ms=3.0, linewidth=1, label='object')
    pl.plot(wave[:1004], sky[:1004], "b-", mfc="none",
            ms=3.0, linewidth=1, label='sky')
    pl.plot(wave[:1004], noise[:1004], "r-", mfc="none",
            ms=3.0, linewidth=1, label='noise (1 sigma)')
...
def wavelengthCalFitsTable(outpath, base_name, order,
                           col, source, wave_exp, wave_fit, res, peak, slope):
    prihdr = fits.Header()
    ...
    tbhdu = fits.BinTableHDU.from_columns(
        fits.ColDefs([
            ...
            fits.Column(
                name='wave_exp (Angstroms)',
                format='1D',
                array=wave_exp),
            fits.Column(
                name='wave_fit (Angstroms)',
                format='1D',
                array=wave_fit),
