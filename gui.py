...
class nsdrpGui():
    def __init__(self):
        ...
        self.plotFrame = Frame(self.gui, bg='lavender') #, width=600, height=500)
        ...
        if os.path.isdir(self.defaultOutDir):
            if os.path.isdir(self.defaultOutDir + '/' + self.utDate):
                self.outDirVar = StringVar(value=self.defaultOutDir + '/' + self.utDate)
                self.batchOutVar = self.outDirVar
            else:
                self.outDirVar = StringVar(value=os.makedirs(self.defaultOutDir + '/' + self.utDate))
                self.batchOutVar = self.outDirVar
        else:	
    ...
    def make_labels(self):
        ...
        self.objectFileLabel = Label(self.mainFrame, text='Object filename:', font=self.labelsFont)
        self.objectFileLabel.grid(row=1, column=0, padx=5, sticky='W')

        self.flatLabel = Label(self.mainFrame, text='Flat filename:', font=self.labelsFont)
        self.flatLabel.grid(row=3, column=0, padx=5, sticky='W')
        
        self.aFrameFileLabel = Label(self.mainFrame, text='A Frame Filename:', font=self.labelsFont)
        self.bFrameFileLabel = Label(self.mainFrame, text='B Frame Filename:', font=self.labelsFont)
        self.pairFlatLabel = Label(self.mainFrame, text='Flat Filename', font=self.labelsFont)
        
        self.batchInputLabel = Label(self.mainFrame, text='Input Directory:', font=self.labelsFont)
        self.batchOutputLabel = Label(self.mainFrame, text='Output Directory:', font=self.labelsFont)

        self.cmdOptionsLabel = Label(self.mainFrame, text='Options:', font=self.labelsFont)

        self.outDirLabel = Label(self.mainFrame, text='Output Directory:', font=self.labelsFont)
        self.outDirLabel.grid(row=5, column=0, sticky='W')

        self.cmdOptionsLabel.grid(row=7, column=0, sticky='W')

    def make_entries(self):
        self.outDirEntry = Entry(self.mainFrame, textvariable=self.outDirVar, width=len(self.outDirVar.get()))
        self.outDirEntry.grid(row=6, column=0, sticky='W')
        
        self.batchInputEntry = Entry(self.mainFrame, textvariable=self.batchInVar, width=len(self.batchInVar.get()))
        self.batchOutputEntry = Entry(self.mainFrame, textvariable=self.batchOutVar, width=len(self.batchOutVar.get()))
    ...
    def make_dropboxes(self):    
        ...
        self.objectFileDropBox = OptionMenu(self.mainFrame, self.objectVar, *self.fitsFiles)
        self.objectFileDropBox.configure(width=self.width)
        self.objectFileDropBox.grid(row=2, column=0, sticky='W')

        self.aFrameDropBox = OptionMenu(self.mainFrame, self.aVar, *self.fitsFiles)
        self.aFrameDropBox.configure(width=self.width)
                                     
        self.bFrameDropBox = OptionMenu(self.mainFrame, self.bVar, *self.fitsFiles)
        self.bFrameDropBox.configure(width=self.width)

        self.flatFileDropBox = OptionMenu(self.mainFrame, self.flatVar, *self.fitsFiles)
...
#       self.gui.after(5000, self.make_dropboxes)
    ...
    def make_buttons(self):
        self.objectButton = Button(self.mainFrame, text='Select Object', command=self.select_object)
        self.objectButton.grid(row=2, column=1, sticky='W')
        self.objectFileDropBox.grid(row=2, column=0, sticky='W')

        self.flatButton = Button(self.mainFrame, text='Select Flat', command=self.select_flat)
        self.flatButton.grid(row=4, column=1, sticky='W')
        self.flatFileDropBox.grid(row=4, column=0, sticky='W')
        
        self.aFrameButton = Button(self.mainFrame, text='Select A Frame', command=self.select_a)
        self.bFrameButton = Button(self.mainFrame, text='Select B Frame', command=self.select_b)

        self.pairFlatButton =  Button(self.mainFrame, text='Select Flat', command=self.select_pair_flat)

        self.outputDirButton = Button(self.mainFrame, text='Select', command=self.update_outdir)
        self.outputDirButton.grid(row=6, column=1, sticky='W')

        self.batchInputButton = Button(self.mainFrame, text='Select', command=self.update_batch_indir)
        self.batchOutputButton = Button(self.mainFrame, text='Select', command=self.update_batch_outdir)

        self.cosmicOption = Checkbutton(self.mainFrame, text='Disable Cosmic', onvalue='-no_cosmic', offvalue='', variable=self.var1)
        self.cosmicOption.grid(row=8, column=0, sticky='W')
        self.jpegOption = Checkbutton(self.mainFrame, text='JPG', onvalue='-jpg', offvalue='', variable=self.var2)
        self.jpegOption.grid(row=9, column=0, sticky='W')

        self.runButton = Button(self.mainFrame, text='Run NSDRP', command=self.go)
        self.runButton.configure(width=10)
        self.runButton.grid(row=10, column=0, sticky='W')
        
        self.plotButton = Button(self.mainFrame, text='Show Plot', command=self.show_plot)

        self.normalModeButton = Radiobutton(self.radioButtonFrame, text='Single Frame', variable=self.modeVar, value=1, command=self.get_mode).grid(row=0, column=0, sticky='W')
        self.pairModeButton = Radiobutton(self.radioButtonFrame, text='Pair Subtraction', variable=self.modeVar, value=2, command=self.get_mode).grid(row=0, column=1, sticky='W')
        self.modeVar.set(1)
        self.batchButton = Radiobutton(self.radioButtonFrame, text='Batch', variable=self.modeVar, value=3, command=self.get_mode).grid(row=0, column=2, sticky='W')
...
#                self.runButton.grid(row=10, column=0, sticky='W')
    ...
    def show_plot(self):
        try:
            ...
            self.img = imopen.open(self.productsDir + '/previews/' + self.productVar.get() + '/' + self.previews[self.previewsListBox.curselection()[0]])
    ...
    def func(self, value):
        self.previews = (os.listdir(self.productsDir + '/previews/' + self.productVar.get()))
    ...
    def products(self):
        ...
        if self.err:
            ...
            self.statusLabel.configure(text='Critical Error:' + ' '.join(self.err), fg='red')
            self.statusLabel.grid(row=self.statusRow, column=1, sticky='W')
        elif not self.err:
        #self.dataProducts:
            self.productsLabel = Label(self.mainFrame, text='Data Products:', font=self.labelsFont)
            self.productsLabel.grid(row=13, column=0, sticky='W')
            self.productsDropBox = OptionMenu(self.mainFrame, self.productVar, *self.dataProducts, command=self.func)
            self.productsDropBox.configure(width=10)
            self.productsDropBox.grid(row=14, column=0, sticky='W')
        else:
            self.statusLabel.configure(text='No data products. Check the log.', fg='black')
            self.statusLabel.grid(row=self.statusRow, column=1, sticky='W')	
    ...
    def go(self):        
        ...
        if os.path.exists(self.productsDir):
            if self.modeVar.get() == 2:
                self.pairFiles = [self.aVar.get(), self.bVar.get(), self.pairFlatVar.get()]
                if len(self.aVar.get()) == 0 or len(self.bVar.get()) == 0 or len(self.pairFlatVar.get()) == 0 or self.aVar.get() == self.missingFile or self.bVar.get() == self.missingFile or self.pairFlatVar.get() == self.missingFile:
                    self.statusLabel.configure(text='Missing a filename!', fg='red')
                    ...
                    self.pairObject = fnmatch.filter(self.files, self.objectPattern)[0]
                    self.bFrame = fnmatch.filter(self.files, self.bFramePattern)[0]
                    self.pairFlat = fnmatch.filter(self.files, self.pairFlatPattern)[0]
                    
                    self.pairCmd = 'python nsdrp.py -out_dir ' + self.productsDir + ' ' + self.var1.get() + ' ' + self.var2.get() + ' ' + self.pairFlat + ' ' + self.pairObject + ' -b ' + self.bFrame
                    
                    self.execute = sp.Popen([self.pairCmd], stdout=sp.PIPE, stderr=sp.PIPE, shell=True)
                ...
                if len(self.objectVar.get()) == 0 or len(self.flatVar.get()) == 0 or self.objectVar.get() == self.missingFile or self.flatVar.get() == self.missingFile:
                    self.statusLabel.configure(text='Missing a filename!', fg='red')
                    ...
                    self.object = fnmatch.filter(self.files, self.objectPattern)[0]
                    self.flat = fnmatch.filter(self.files, self.flatPattern)[0]

                    self.cmd = 'python nsdrp.py -out_dir ' + self.productsDir + ' ' + self.var1.get() + ' ' + self.var2.get() + ' ' + self.object + ' ' + self.flat
                    
                    self.execute = sp.Popen([self.cmd], stdout=sp.PIPE, stderr=sp.PIPE, shell=True)
                ...
                if len(self.batchInVar.get()) == 0 or len(self.batchOutVar.get()) == 0:
                    self.statusLabel.configure(text='Missing a directory', fg='red')
                    self.statusLabel.grid(row=10, column=0, sticky='E')
                #elif not os.path.isdir(self.batchInVar.get()) or not os.path.isdir(self.batchOutVar.get()):
                elif not os.path.isdir(self.batchInVar.get()):
                    self.statusLabel.configure(text='Input directory does not exists', fg='red')
                    self.statusLabel.grid(row=10, column=0, sticky='E')
                else:
                    self.runButton.configure(state=DISABLED, relief=SUNKEN)
                    
                    self.batchCmd = 'python nsdrp.py ' + self.var1.get() + ' ' + self.var2.get() + ' ' + self.batchInVar.get() + ' ' + self.batchOutVar.get()

                    self.execute = sp.Popen([self.batchCmd], stdout=sp.PIPE, stderr=sp.PIPE, shell=True)
