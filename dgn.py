...
def gen(reduced, out_dir):
    ...
    logger.info(
        'generating diagnostic data products for {}...'.format(
            reduced.getBaseName()))
    ...
    tops_bots_plot(
        out_dir,
        reduced.Flat.baseName,
        reduced.Flat.topEdgeImg,
        reduced.Flat.botEdgeImg)
...
def constructFileName(outpath, base_name, order, fn_suffix):
    if order is None:
        return outpath + '/diagnostics/' + \
            subdirs[fn_suffix] + '/' + base_name + '_' + fn_suffix
    else:
...
def edges_plot(outpath, base_name, top_profile,
               bot_profile, top_peaks, bot_peaks):
...
def tops_bots_plot(outpath, base_name, tops, bots):
    ...
    pl.cla()
    pl.suptitle(
        'top and bottom order edges, {}'.format(base_name),
        fontsize=14)
#     pl.set_cmap('Blues_r')
...
def traces_plot(outpath, obj_base_name, flat_base_name, order_num, obj_img, flat_img, top_trace,
    ...
    pl.cla()
    pl.suptitle(
        'order edge traces, {}, order {}'.format(
            obj_base_name,
            order_num),
        fontsize=14)
    pl.set_cmap('Blues_r')
...
def order_location_plot(outpath, obj_base_name,
                        flat_base_name, flat_img, obj_img, orders):
...
def spatrect_plot(outpath, base_name, order_num, obj, flat):
    ...
    pl.cla()
    pl.suptitle(
        'spatially rectified, {}, order {}'.format(
            base_name,
            order_num),
        fontsize=14)
    pl.set_cmap('Blues_r')
...
def perOrderWavelengthCalAsciiTable(
        outpath, base_name, order, col, centroid, source, wave_exp, wave_fit, res, peak, slope):
...
#     pl.rcParams['ytick.labelsize'] = 8
    ...
    ymin = np.amin(order.synthesizedSkySpec) - \
        ((np.amax(order.synthesizedSkySpec) - np.amin(order.synthesizedSkySpec)) * 0.1)
    ...
    ymin = np.amin(
        order.skySpec['A']) - ((np.amax(order.skySpec['A']) - np.amin(order.skySpec['A'])) * 0.1)
    ymax = np.amax(
        order.skySpec['A']) + ((np.amax(order.skySpec['A']) - np.amin(order.skySpec['A'])) * 0.1)
