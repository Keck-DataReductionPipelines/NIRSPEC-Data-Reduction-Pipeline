...
#             return trace
    ...
    if nJumps > ORDER_EDGE_JUMP_LIMIT:
        ...
        logger.debug(
            'reducing search width to {:.1f}'.format(
                ORDER_EDGE_SEARCH_WIDTH / 1.5))
        trace, nJumps = tracer.trace_edge(
        ...
        if nJumps > ORDER_EDGE_JUMP_LIMIT:
            ...
            if config.params['spatial_jump_override'] is True:
                logger.warning(
                    'spatial jump override enabled, edge not rejected')
            else:
...
#     pl.show()
    ...
    if centroid_sky_sum.any():
        logger.info(
            str(fitnumber) +
            ' sky lines used for spectral rectification')
...
def smooth_spectral_trace(data, l):
    # p0 = np.polyfit(np.arange(len(data) - 10), data[:-10], deg=1)  # end
    # always drops off
