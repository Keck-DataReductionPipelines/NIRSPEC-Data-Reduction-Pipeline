...
def normalize(data, on_order, off_order):
    ...
    # around the edges of the order can blow up when div by mean, set those to
    # one
...
def cosmic_clean(data):
    ...
    try:
        ...
        cosmicRayMask, cleanedArray = astroscrappy.detect_cosmics(data, pssl=0.0, gain=2.2, sigclip=sig_clip, sigfrac=sig_frac,
                                                                  objlim=obj_lim, readnoise=10.0, satlevel=np.inf, inmask=None, sepmed=False, cleantype='medmask', fsmode='median')
    except ImportError:
...
def extract_spectra(obj, flat, noise, obj_range, sky_range_top, sky_range_bot):
    ...
    sky_mean = (sky_top_sum + sky_bot_sum) / \
        (len(sky_range_top) + len(sky_range_bot))
