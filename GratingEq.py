...
class GratingEq:
    ...
    def evaluate(self, order, filtername, slit,
                 echlpos, disppos, dateobs=None):
        """
        ...
        # spatial axis
        left_mid_row = c1 * wavelength_left + \
            c2 * np.sin(np.radians(disppos)) + y0
        ...
        if config.params['sowc'] is True:
            ...
            if '24' in slit:
                ...
                self.logger.info(
                    'long slit order width = {:.0f} pixels'.format(orderWidth))
            else:
                orderWidth = 12.0 / 0.193
                self.logger.info(
                    'short slit order width = {:.0f} pixels'.format(orderWidth))
        ...
        #     date_y_corr = 50
            ...
            if '24' in slit:
                ...
                left_bot_row = left_top_row - \
                    ((left_top_row - left_bot_row) * 2)

            elif '1.13' in slit:
                self.logger.debug(
                    'applying no correction for x1.13 AO slit correction')
            ...
            if 'NIRSPEC-7' in filtername:
                ...
                self.logger.info(
                    'order width = {:.0f} pixels'.format(
                        left_top_row - left_bot_row))
