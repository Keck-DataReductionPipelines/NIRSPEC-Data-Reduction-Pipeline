...
def extract_order(order_num, obj, flat, top_calc,
                  bot_calc, filter_name, slit_name):
    """
    ...
    # cut out order from object frame and flat and compute on and off order
    # masks
...
def cut_out_order(obj, flat, order):
    ...
    order.objCutout = np.array(
        cut_out(
            obj,
            order.highestPoint,
            order.lowestPoint,
            order.padding))
    order.flatCutout = np.array(
        cut_out(
            flat,
            order.highestPoint,
            order.lowestPoint,
    ...
    order.objCutout = np.ma.masked_array(
        order.objCutout, mask=order.offOrderMask)
    order.flatCutout = np.ma.masked_array(
        order.flatCutout, mask=order.offOrderMask)
...
def find_edge_traces(tops, bots, order, slit_name):
    ...
    if order.topMeas is not None:
        ...
        order.topTrace = nirspec_lib.trace_order_edge(
            tops, order.topMeas, slit_name)

    if order.botMeas is not None:
        logger.debug('tracing bottom of order')
        order.botTrace = nirspec_lib.trace_order_edge(
            bots, order.botMeas, slit_name)
...
def determine_edge_locations(tops, bots, order, min_intensity, max_delta):
    ...
    if order.topMeas is not None:
        if (order.topMeas < 1) or (
                abs(order.topMeas - order.topCalc) > (max_delta)):
    ...
    if order.botMeas is not None:
        if (order.botMeas < 1) or (
                abs(order.botMeas - order.botCalc) > (max_delta)):
